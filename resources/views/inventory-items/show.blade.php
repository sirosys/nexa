...
<x-app-layout :title="'Detail Item Inventaris — ' . config('app.name', 'NEXA')">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            <a href="{{ route('inventory-items.index') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:underline"><x-icon name="arrow-left" size="4" />Kembali ke Inventaris</a>
            <h1 class="mt-1 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $item->product?->name }}</h1>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($item->units as $unit)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3">
                                            <span class="inline-flex items-center rounded-full {{ $unitStatusClasses[$unit->status] ?? 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' }} px-3 py-1 text-[13px] font-semibold">
            ...
            @can('update', $item)
                <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-[0_0_20px_0_rgba(76,87,125,0.02)] dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <form method="POST" action="{{ route('inventory-items.stock-in', $item) }}" class="space-y-3">
                        ...
                        <button type="submit" class="w-full rounded-xl bg-success px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-success/25 transition hover:bg-success-active hover:shadow-md active:scale-[0.98]">Tambah Stok</button>
                    </form>
                </div>
