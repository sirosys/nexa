<x-app-layout :title="'Ubah Coverage — ' . config('app.name', 'NEXA')">
    <div class="mb-6">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Ubah Coverage</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kode: {{ $coverage->code }}</p>
    </div>
    ...
    <div class="max-w-xl rounded-2xl border border-gray-200 bg-white p-6 shadow-[0_0_20px_0_rgba(76,87,125,0.02)] dark:border-gray-700 dark:bg-gray-800">
        <form method="POST" action="{{ route('coverages.update', $coverage) }}">
            @csrf
            @method('PUT')
