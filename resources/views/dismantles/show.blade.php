...
<x-app-layout :title="'Dismantle — ' . $service->code . ' — ' . config('app.name', 'NEXA')">
    <div class="mb-6">
        <a href="{{ route('dismantles.index') }}" class="text-sm font-medium text-primary hover:underline">&larr; Kembali ke Dismantle</a>
            ...
            @if ($service->status === \App\Models\Service::STATUS_PENDING_DISMANTLE && $viewer->isTechnician())
                <div class="rounded-2xl border border-gray-300 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <form method="POST" action="{{ route('dismantles.claim', $service) }}">
                        @csrf
                        <button type="submit" class="w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-active">Klaim Dismantle Ini</button>
                    </form>
                </div>
            @endif
            ...
            @if ($service->status === \App\Models\Service::STATUS_DISMANTLING && $viewer->isTechnician() && $dismantle?->technician_id === $viewer->id)
                <div class="rounded-2xl border border-gray-300 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <form method="POST" action="{{ route('dismantles.complete', $service) }}" enctype="multipart/form-data" class="space-y-3">
                        ...
                        <button type="submit" class="w-full rounded-lg bg-success px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-success-active">Selesaikan Dismantle</button>
                    </form>
                </div>
            @endif
        </div>
