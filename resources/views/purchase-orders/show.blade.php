...
<x-app-layout :title="'Detail Purchase Order — ' . config('app.name', 'NEXA')">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            <a href="{{ route('purchase-orders.index') }}" class="text-sm font-medium text-primary hover:underline">&larr; Kembali ke Purchase Order</a>
            <h1 class="mt-1 text-xl font-semibold text-gray-900 dark:text-white">{{ $purchaseOrder->code }}</h1>
            ...
            @if ($purchaseOrder->status === PurchaseOrder::STATUS_DRAFT)
                @can('update', $purchaseOrder)
                    <a
                        ...
                        class="rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700"
                    >
                        Ubah
                    </a>
                @endcan
                @can('order', $purchaseOrder)
                    <form method="POST" action="{{ route('purchase-orders.order', $purchaseOrder) }}" onsubmit="return confirm('Tandai Purchase Order ini sebagai dipesan ke vendor?');">
                        @csrf
                        <button type="submit" class="rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-active">Tandai Dipesan</button>
                    </form>
                @endcan
            @endif
            ...
            @if (in_array($purchaseOrder->status, [PurchaseOrder::STATUS_DRAFT, PurchaseOrder::STATUS_ORDERED], true))
                @can('cancel', $purchaseOrder)
                    <form method="POST" action="{{ route('purchase-orders.cancel', $purchaseOrder) }}" onsubmit="return confirm('Batalkan Purchase Order ini?');">
                        @csrf
                        <button type="submit" class="rounded-lg border border-danger/30 px-4 py-2.5 text-sm font-semibold text-danger transition hover:bg-danger-light dark:hover:bg-danger/10">Batalkan</button>
                    </form>
                @endcan
            @endif
        </div>
                ...
                <div class="space-y-6">
                    <div class="rounded-2xl border border-gray-300 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <form method="POST" action="{{ route('purchase-orders.receive', $purchaseOrder) }}" class="space-y-4">
                            ...
                            <button type="submit" class="w-full rounded-lg bg-success px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-success-active">Terima Barang</button>
                        </form>
                    </div>
                </div>
            @endcan
