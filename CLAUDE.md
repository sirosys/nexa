...
- **Satu tabel `users` untuk semua role** (dibedakan flag `admin` — belum ada kolom "role" granular; itu tetap item roadmap Role & Permission terpisah). Tidak ada lagi subclass `Customer` — Controller/Service/Request/Policy semua bertipe `App\Models\User` langsung.
- **Toggle "Admin / staff"** ada di form create/edit (`resources/views/users/_form.blade.php`, checkbox `name="admin"`) — jadi admin yang login sekarang bisa membuat/menandai akun staff lewat UI yang sama, bukan cuma lewat `AdminUserSeeder`. **Guard yang sudah ada**: admin tidak bisa mencabut status admin dari akun miliknya sendiri (`UserRequest::guardAgainstSelfDemotion()`) — supaya tidak keunci sendiri dari sistem selama belum ada akun admin lain/mekanisme recovery. Guard lain (mis. mencegah admin biasa membuat/menghapus admin lain, atau tier "superadmin") **belum diimplementasikan** — user sudah bilang eksplisit itu "dipikirkan lagi nanti" sesuai kebutuhan; jangan improvisasi skema permission granular sebelum diminta.
- **`users.code`** hanya digenerate untuk akun **non-admin** (`'CUS'.str_pad($user->id, 6, '0', STR_PAD_LEFT)` di `UserService::create()`, setelah insert — aman dari race condition karena mengandalkan auto-increment id) — akun admin/staff dibiarkan `code = null` karena `code` itu untuk keperluan invoicing/billing customer (lihat draft ERD), staff tidak butuh.
...
- **Authorization**: `App\Policies\UserPolicy` (viewAny/view/create/update/delete → `$user->isAdmin()`) — Laravel auto-discover lewat konvensi nama (`User` model ↔ `UserPolicy`, tidak perlu atribut `#[UsePolicy]` eksplisit lagi seperti versi `Customer` sebelumnya), dipanggil dari `UserController::__construct()` via `$this->authorizeResource(User::class, 'user')`. `App\Http\Controllers\Controller` (base class) di-extend dari `Illuminate\Routing\Controller` (`$this->middleware()`) + `use AuthorizesRequests` supaya `authorizeResource()` jalan — masih didukung penuh routing layer Laravel 13.
...
- Test: `tests/Feature/UserManagementTest.php` — CRUD admin, code auto-generate hanya utk non-admin, listing gabungan customer+staff, guard self-demotion, validasi unique phone/nik, NIK lock, non-admin dapat 403, dan akses foto KTP privat.
...
- Serving lewat `GET /secure/ktp/{user}` → `App\Http\Controllers\KtpPhotoController@show`, route name `secure.ktp`, terdaftar di grup middleware `auth`. Otorisasi inline (bukan Policy — cukup sederhana untuk 1 aksi): boleh diakses **pemilik akun sendiri** (`$viewer->id === $user->id`) **atau admin/staff** (`$viewer->isAdmin()` — berperan sebagai "superadmin/staff" yang disebut user, karena tingkatan superadmin terpisah belum ada; lihat catatan guard di atas). Selain itu → 403. Kalau `ktp_photo` null → 404. Response pakai `Storage::disk('local')->response($path)` (inline, bukan `download()`, supaya `<img>` tag bisa render langsung).
...
- `users` — akun (customer dan/atau staff, dibedakan lewat flag `admin`), dengan phone/code, password, `last_login_at`. Data KTP/dob/gender **tidak** lagi di kolom `users` langsung di draft ini — sudah dipisah ke tabel `user_details` (lihat poin berikutnya).
...
Ada file drawio kedua, `database/database_actual.drawio`, dengan tujuan berbeda dari diagram desain di atas: itu bukan draft, melainkan cermin skema yang **sungguhan** sudah dimigrasikan ke database saat ini (`users`, `otp_codes`, `subdistricts`, `user_details`), kolomnya diambil langsung dari migration — bukan dari diagram desain, karena keduanya bisa saja menyimpang. Wajib di-update tiap kali ada migration baru landing, sama seperti kewajiban update CLAUDE.md ini sendiri.
